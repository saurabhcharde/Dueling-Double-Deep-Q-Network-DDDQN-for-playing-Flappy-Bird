...
class DQN():
    ...
    def build_model(self):
        ...
        model=Sequential()
        model.add(Dense(units=64,input_dim=self.state_space,activation='relu'))
        model.add(Dense(units=64,activation='relu'))
        model.add(Dense(units=64,activation='relu'))
        #self.action_space
        y=Dense(units=self.action_space+1,activation='linear')(model.layers[-1].output)
        outputlayer = Lambda(lambda a: K.expand_dims(a[:, 0], -1) + a[:, 1:] - K.mean(a[:, 1:], keepdims=True), output_shape=(self.action_space,))(y)
...
if __name__=="__main__":
    ...
    try:
        ...
        #agent.model.load_weights('Flappy_dddqn.h5')
        final_score=0
        y=list()
        x=list()
